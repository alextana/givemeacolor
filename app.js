/* PART OF THE COLOUR PALETTE GENERATOR
====== GET A COLLECTION OF COLOURS BASED ON THE SHADE ======
Given a number of x basic colours, display a larger amount of shades for
people to choose from, speeding up the process of selecting colours
before starting a concept

version 1.0  
Using rgb to hex and rgb to hsl from css-tricks.com
Array of colours taken from Name That Color's Source Code ->  http://chir.ag/projects/ntc/ntc.js


// temporary comment for the building process
...
function pushColorsToArrays() {
    ...
    for (var y =0;y<names.length;y++) {
        let r = 0,
            ...
            b = 0;
        // convert hex to rgb
        if (names[y][0].length == 6) {
            r = "0x" + names[y][0][0] + names[y][0][1];
            g = "0x" + names[y][0][2] + names[y][0][3];
            b = "0x" + names[y][0][4] + names[y][0][5];
        }
        const hslConvert = () => {
        ...
        // output rgb color in a readable way and as comparable way
        let rgbColor, comparableRgbColor, hslColor, hslColorLuminance, hslColorHue, hslColorSaturation,
            hexColor;
            rgbColor = "rgb(" + +r + "," + +g + "," + +b + ")",
            comparableRgbColor = [+r, +g, +b];
            hslColor = hslConvert();
            hslColorLuminance = hslColor[2];
            hslColorSaturation = hslColor[1];
            hslColorHue = hslColor[0];
            // outputs hex values for the frontend
            /* === FUTURE TO DO === */
            // can think of a way to display various conversions
            hexColor = '#' + names[y][0];
        // function that pushes colours into respective spectrum arrays
        // based on hue and luminance values
        const divideColors = (spectrumArray) => {
            spectrumArray.push(hexColor);
        }
        const colorDivider = (minHue, maxHue, darkArray, balancedArray, lightArray) => {
            // conditional for dark colors
            ...
            // conditional for Light colors
                ...
                hslColorLuminance >= 58 && hslColorLuminance <= 98 && hslColorSaturation > 50 && hslColorSaturation !== 0) {
                divideColors(lightArray);
            }
        }
        // special case gray 
        // gray doesn't change in hue but in saturation so it needs to be outside the function
        if (hslColorLuminance > 10 && hslColorLuminance <= 40 && hslColorSaturation === 0) {
            divideColors(darkGray);
        } else if (hslColorLuminance > 40 && hslColorLuminance < 58 && hslColorSaturation === 0) {
            divideColors(gray);
        } else if (hslColorLuminance >= 58 && hslColorLuminance <= 92 && hslColorSaturation === 0) {
            divideColors(lightGray);
        }
        // end special case gray 
        // special cases white and black
        else if (hslColorLuminance > 92 && hslColorSaturation === 0) {
            divideColors(white);
        } else if (hslColorLuminance >= 0 && hslColorLuminance <= 10 && hslColorSaturation === 0) {
            divideColors(black);
        }
        // by changing the hue this should divide the colors nicely
        // needs some tweaking
...
const outputColors = (selectedArray) => {
    for (var x = 0; x < selectedArray.length; x++) {
        ...
        colorBox.addEventListener('click', function(){
            ...
            // append copied to clipboard to existing text
